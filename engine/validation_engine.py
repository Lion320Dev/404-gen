...
class ValidationEngine:
    ...
    def _custom_compute_image_based_metrics(self, images: list[torch.Tensor], mean_op: str) -> tuple[float, float, float]:
        """Function that computes image based metrics"""
    ...
    def validate_custom_text_to_gs(self, prompt: str, images: list[torch.Tensor], mean_op: str = "") -> ValidationResult:
        """Function that validates the input 3D data generated using provided prompt"""

        combined_quality_score= self._custom_compute_image_based_metrics(images, mean_op)

        validation_results = ValidationResult(
            final_score=0,
            combined_quality_score=combined_quality_score,
            alignment_score=0,  # artificial normalization for current clip version
            ssim_score=0,
            lpips_score=0,
        )
        return self._compute_final_score(validation_results)
